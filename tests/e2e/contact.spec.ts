...
test("valid contact submission succeeds with Telegram disabled", async ({ page }) => {
  await page.goto("/en/contact")

  await page.locator("#contact-name").fill("Playwright Test")
  await page.locator("#contact-email").fill("playwright@example.com")
  await page.locator("#contact-message").fill("Deterministic E2E message that must not reach Telegram.")
  await page.getByRole("button", { name: /send/i }).click()
