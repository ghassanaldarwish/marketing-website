---
title: "Aufbau einer produktionsreifen AI-Agent-Plattform"
description: "Wie ich eine Agentenplattform mit Node.js, Python, Redis, PostgreSQL, Docker und Kubernetes entworfen habe."
...
coverImageAlt: "Architektur einer produktionsreifen AI-Agent-Plattform"
...
Diese Fallstudie beschreibt die Architektur einer produktionsorientierten AI-Agent-Plattform, die mit **TypeScript**, **Python**, **Redis**, **PostgreSQL**, **Docker** und **Kubernetes** entwickelt wurde.

Das Ziel war nicht, einen weiteren Chatbot zu entwickeln. Stattdessen sollte eine wiederverwendbare Plattform entstehen, auf der intelligente, toolgestützte Workflows innerhalb eines zuverlässigen Backend-Systems ausgeführt werden können.
...
Die Plattform musste Folgendes unterstützen:
...
Die zentrale Herausforderung bestand darin, probabilistisches AI-Reasoning mit deterministischer Backend-Infrastruktur zu verbinden.
...
Die Architektur musste das AI-Reasoning daher von den Systemteilen isolieren, für die strikte Garantien erforderlich waren.

## Designziele

Vor der Auswahl der Technologien definierte ich die wichtigsten Designziele.
...
Die Plattform ist in mehrere klar definierte Ebenen unterteilt.

![Architektur der AI-Agent-Plattform](/articles/ai-agent-platform/architecture.png)
...
## Warum ich API und Agent-Ausführung getrennt habe
...
Stattdessen erstellt die API einen dauerhaften Run und plant die asynchrone Verarbeitung.
...
Die API veröffentlicht innerhalb der Transaktion nicht unmittelbar eine Nachricht in Redis. Stattdessen speichert sie ein Outbox-Ereignis.
...
Die transaktionale Outbox löst dieses Problem, indem Workflow und Ereignis innerhalb derselben PostgreSQL-Transaktion gespeichert werden.

Ein separater Publisher liest unverarbeitete Outbox-Ereignisse und veröffentlicht sie in Redis.
...
Das Outbox-Pattern stellt sicher, dass ein bestätigter Workflow zwischen Datenbank und Queue nicht unbemerkt verloren gehen kann.
...
PostgreSQL speichert den dauerhaften Zustand jedes Runs.
...
Dadurch erhält die Plattform einen auditierbaren Ausführungsverlauf.

Außerdem kann ein Worker nach einem Neustart ab dem letzten abgeschlossenen Schritt fortfahren, statt den gesamten Workflow erneut zu starten.

## Design des Agent-Workers

Der Python-Worker ist für die Modellinteraktion, strukturierte Ausgaben und Reasoning-bezogene Operationen verantwortlich.
...
Der Worker erhält eine Job-ID und lädt den aktuellen Zustand erneut aus PostgreSQL.
...
Fehlerhaft formatierte Ausgaben werden vom System abgelehnt, anstatt die Absicht des Modells zu erraten.
...
Das Modell erhält niemals direkte Zugangsdaten.
...
Anschließend führt das Backend mehrere Prüfungen durch:
...
Das Modell schlägt eine Aktion vor. Die Anwendung autorisiert, validiert, führt sie aus und protokolliert sie.
...
Jede Operation mit Seiteneffekten erhält einen Idempotenzschlüssel:
...
Dadurch kann der Worker die Ausführung sicher wiederholen.

## Aufgaben von Redis

Redis wird für kurzlebige operative Aufgaben eingesetzt:
...
Redis wird nicht als einziger Speicherort für geschäftskritische Workflow-Zustände verwendet.
