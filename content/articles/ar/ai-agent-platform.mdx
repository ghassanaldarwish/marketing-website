---
title: "بناء منصة وكلاء ذكاء اصطناعي جاهزة للإنتاج"
description: "كيف صممت منصة للوكلاء باستخدام Node.js وPython وRedis وPostgreSQL وDocker وKubernetes."
...
coverImageAlt: "معمارية منصة وكلاء ذكاء اصطناعي جاهزة للإنتاج"
...
تصف دراسة الحالة هذه معمارية منصة وكلاء ذكاء اصطناعي موجهة لبيئات الإنتاج، بُنيت باستخدام **TypeScript** و**Python** و**Redis** و**PostgreSQL** و**Docker** و**Kubernetes**.

لم يكن الهدف بناء روبوت محادثة آخر، بل إنشاء منصة قابلة لإعادة الاستخدام لتشغيل تدفقات عمل ذكية تستخدم الأدوات داخل نظام خلفي موثوق.
...
لذلك كان على المعمارية عزل استدلال الذكاء الاصطناعي عن أجزاء النظام التي تتطلب ضمانات صارمة.

## أهداف التصميم

قبل اختيار التقنيات، حددت أهداف التصميم الأساسية.
...
تنقسم المنصة إلى عدة طبقات محددة بوضوح.
...
## لماذا فصلت API عن تنفيذ الوكيل
...
بدلًا من ذلك، تنشئ API عملية تشغيل دائمة وتجدول عملًا غير متزامن.
...
لا تنشر API رسالة مباشرة إلى Redis داخل المعاملة، بل تخزن حدثًا في صندوق الصادر.
...
يحل نمط صندوق الصادر المعاملي هذه المشكلة من خلال كتابة تدفق العمل والحدث ضمن معاملة PostgreSQL نفسها.
...
يضمن نمط صندوق الصادر ألا يختفي تدفق عمل تم تثبيته في قاعدة البيانات بصمت في المسافة بين قاعدة البيانات والطابور.
...
يخزن PostgreSQL الحالة الدائمة لكل عملية تشغيل.
...
يمنح هذا المنصة سجل تنفيذ قابلًا للتدقيق.

ويعني أيضًا أن العامل يستطيع إعادة التشغيل والمتابعة من آخر خطوة مكتملة بدلًا من بدء تدفق العمل بالكامل من جديد.

## تصميم عامل الوكيل

يتولى عامل Python مسؤولية التفاعل مع النموذج، والمخرجات المنظمة، والعمليات المرتبطة بالاستدلال.
...
يتلقى العامل معرّف المهمة ثم يعيد تحميل الحالة الحالية من PostgreSQL.
...
يرفض النظام المخرجات غير المطابقة للبنية بدلًا من محاولة تخمين ما قصده النموذج.
...
لا يتلقى النموذج بيانات اعتماد مباشرة مطلقًا.
...
ينفذ النظام الخلفي بعد ذلك عدة فحوصات:
...
يقترح النموذج إجراءً، بينما يتولى التطبيق التفويض والتحقق والتنفيذ والتسجيل.
...
تحصل كل عملية تُحدث أثرًا جانبيًا على مفتاح عدم تكرار:
...
يسمح ذلك للعامل بإعادة المحاولة بأمان.

## مسؤوليات Redis

يُستخدم Redis للاحتياجات التشغيلية قصيرة العمر:
...
لا يُستخدم Redis بوصفه موقع التخزين الوحيد لحالة تدفق العمل الحساسة للأعمال.
